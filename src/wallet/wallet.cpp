...
/** 
 * Fees smaller than this (in edens) are considered zero fee (for transaction creation)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == MNCollateral*COIN);
                    ...
                    found = !CPrivateSend::IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fMasterNode) found = pcoin->vout[i].nValue != MNCollateral*COIN; // do not use Hot MN funds
                } else if(nCoinType == ONLY_2500) {
                    found = pcoin->vout[i].nValue == MNCollateral*COIN;
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT2500IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 2500 EDEN.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT2500IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 2500 EDEN.");
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 EDEN."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-eden-address
                        CScript scriptChange;
