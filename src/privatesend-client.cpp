...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Eden related functionality
...
bool CPrivateSendClient::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated, CConnman& connman)
{
    ...
    if(!fSuccess) {
        LogPrintf("CPrivateSendClient::MakeCollateralAmounts -- ONLY_NONDENOMINATED_NOT2500IFMN: %s\n", strFail);
        ...
        if(fTryDenominated) {
            ...
            if(!pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                                nFeeRet, nChangePosRet, strFail, &coinControl, true, ONLY_NOT2500IFMN)) {
                LogPrintf("CPrivateSendClient::MakeCollateralAmounts -- NOT2500IFMN Error: %s\n", strFail);
...
{
    if(fLiteMode) return; // disable all Eden specific functionality
