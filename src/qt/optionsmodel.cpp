...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("fHideNotifications"))
        settings.setValue("fHideNotifications", false);
    fHideNotifications = settings.value("fHideNotifications").toBool();

    // Display
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EDEN);
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeEdenAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeEdenAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eden.cpp during shotdown to also remove the window positions
